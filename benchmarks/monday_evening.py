	...
	for optionmysqlprovide in [3]:
		...
		for solver in ["g12", "gecode"]:
			for optionwebservers in [0,1,3,4]:
				...
				ax = fig.add_subplot(2, 4, 1 + webserver_indices[optionwebservers] + (solver_indices[solver] * 4), projection='3d')
				#ax.set_title("Solver: %s\nMySQL provide: %d\nWebservers: %d" % (solver, optionmysqlprovide, optionwebservers))
				ax.set_title("Solver: %s\nWebservers: %d" % (solver, optionwebservers))
		...
		for extension in ["eps", "pdf"]:
			fig.savefig('charts_monday_evening.%s' % (extension), bbox_inches='tight', dpi=150)
