...
let conflicts_of_repository (universe : universe) (repository_id : repository_id) : component_type_id list list =
  ...
  let coinst_exit_code = 
    Unix.system (Printf.sprintf "./coinst/coinst -all -conflicts %s -cudf %s" conflicts_filepath cudf_filepath) in
