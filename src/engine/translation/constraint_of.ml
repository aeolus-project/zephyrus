...
let optimization_function_full ?(with_packages = true) (universe : Data_model.universe option) (initial_configuration : Data_model.configuration option) (opt_fun : Data_model.optimization_function option) : Data_constraint.optimization_function option = 
  match opt_fun with
  | None         -> None
  | Some opt_fun -> 
    match (universe, initial_configuration) with
